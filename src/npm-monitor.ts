...
export class NPMMonitor {
  ...
  private async login(): Promise<boolean> {
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        logger.info(
          `Attempting to login to NPM at ${this.apiUrl} (attempt ${
            attempt + 1
          })`
        );
  ...
  private async getProxyHosts(): Promise<NPMHost[]> {
    ...
    if (this.consecutiveFailures >= this.maxConsecutiveFailures) {
      ...
      this.stopMonitoring();
      throw new Error(
        "NPM API is unreachable after multiple attempts. Monitoring stopped."
      );
    }
  ...
  private async getChangesSinceLastCheck(): Promise<{
    ...
    const changedHosts = currentHosts.filter((currentHost) => {
      const previousHost = this.lastKnownHosts.find(
        (h) => h.id === currentHost.id
      );
      if (!previousHost) {
      ...
      const hasChanged =
        new Date(currentHost.modified_on).getTime() !==
          new Date(previousHost.modified_on).getTime() ||
        currentHost.forward_host !== previousHost.forward_host ||
        currentHost.forward_port !== previousHost.forward_port ||
        JSON.stringify(currentHost.domain_names) !==
          JSON.stringify(previousHost.domain_names);
  ...
  async startMonitoring(
    ...
    logger.info(
      `Starting NPM monitoring with ${this.checkInterval}ms interval`
    );
    ...
    const monitor = async () => {
      try {
        ...
        const { currentHosts, changedHosts, deletedHosts } =
          await this.getChangesSinceLastCheck();
